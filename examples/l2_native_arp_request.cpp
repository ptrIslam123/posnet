...
int main(void) {
    ...
    unsigned char* etherhead = (unsigned char*)buffer;  /*Pointer to Ethenet Header*/
    struct ethhdr *eh = (struct ethhdr *)etherhead; /*Another pointer to
                                                    ethernet header*/
    unsigned char* arphead = (unsigned char*)buffer + 14;
    ...
    while (1) {
        ...
        if(htons(eh->h_proto) == ETH_P_ARP)
        {
            ...
            if (sent == -1)
            {
                ...
                printf("sent the arp packet successful!\n");
            }
